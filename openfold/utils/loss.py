...
def supervised_chi_loss(
    ...
    **kwargs,
    ...
    check_inf_nan([sq_chi_error, sq_chi_error_shifted])
...
def kl_loss(latent_mean, latent_logvar, seq_mask, eps=1e-8, **kwargs):
    """
    Computes kl regularization loss.
    prior N(0, I), posterior(mu, sigma^2)
    ...
    # [*, ]
    loss = 0.5 * (
        torch.sum(torch.exp(latent_logvar), dim=-1) + \
        torch.sum(torch.pow(latent_mean, 2), dim=-1) - \
        torch.sum(latent_logvar + 1.0, dim=-1)
    )
    # loss = loss * seq_mask

    # loss = torch.sum(loss, dim=-1)
    # denom = eps + torch.sum(seq_mask, dim=-1)
    # loss = loss / denom
    loss = torch.mean(loss)
...
class AlphaFoldLoss(nn.Module):
    ...
    def forward(self, out, batch, _return_breakdown=False):
        ...
        loss_fns = {
            ...
            "latent_distogram": lambda: latent_distogram_loss(
                logits=out["latent_distogram_logits"],
                **{**batch, **self.config.latent_distogram},
            ),
