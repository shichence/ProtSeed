...
def main(args):
    ...
    pdb_ids = [x for x in os.listdir(args.pdb_dir) if x.endswith('.pdb')]
    ...
    for pdb_id in pdb_ids:
        # Get input
        pdb_path = os.path.join(args.pdb_dir, pdb_id)
        ...
        try:
            logging.info(f"Executing model...tag: {name}")
            ...
            logging.info(f"fail to generate {name} due to {e}")
            continue
        ...
        res_dict[top].update({name: [aar, rmsd, ppl]})
        all_aar.append(aar)
        ...
        unrelaxed_output_path = os.path.join(
            args.output_dir,
            f"{name}_{aar}_{rmsd}_{ckpt_epoch}_{args.config_preset}_rec{args.no_recycling_iters}_s{args.seed}_unrelaxed.pdb"
        )
        ...
        if args.relax:
            ...
            try:
                ...
                relaxed_output_path = os.path.join(
                    args.output_dir,
                    f"{name}_{aar}_{rmsd}_{ckpt_epoch}_{args.config_preset}_rec{args.no_recycling_iters}_s{args.seed}_relaxed.pdb"
                )
    ...
    logging.info(f"mean_aar: {all_aar.mean()} | median_aar: {np.median(all_aar)}")
    logging.info(f"mean_rmsd: {all_rmsd.mean()} | median_rmsd: {np.median(all_rmsd)}")
    logging.info(f"mean_ppl: {all_ppl.mean()} | median_ppl: {np.median(all_ppl)}")
    # with open(os.path.join(args.output_dir, "res_dict.pkl"), 'w') as fout:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--deepspeed", type=bool_type, default=True,
